...
function initSearch(){
	var arr =[];
	arr[arr.length]= $("#provider").val(); //0
	arr[arr.length]= $("#service").val(); //1
	arr[arr.length]= $('select[name="house"] option:selected').text(); //2
	arr[arr.length]= $("#fio").val(); //3
	arr[arr.length]= $("#state").val();  //4
	arr[arr.length]= $("#dfstatus").val(); //5
	arr[arr.length]= $("#dtstatus").val(); //6
	arr[arr.length]= $("#debetf").val();  //7
	arr[arr.length]= $("#debett").val();  //8
	arr[arr.length]= $("#dfdebet").val(); //9
	arr[arr.length]= $("#dtdebet").val(); //10
	arr[arr.length]= $("#ls").val();//11
	arr[arr.length]= $("#dflaw").val();//12
	arr[arr.length]= $("#dtlaw").val(); //13
  ...
  $('#btnSearch').on('click', function(e) {
        var arr=initSearch();
       	$.ajax({
                    ...
                    cache: false,
                    success: function(responce){ $('div[id="search_debet"]').html(responce); }
              });
   })
});  // btnSearch
...
$(function() {
  $('#btnState').on('click', function(e) {
        var x = $('input[name="law[]"]:checked');
		var state_ch = $("#state_ch").val();
		$("#state [value='"+state_ch+"']").attr("selected", "selected");
		var laws = '';
...
$(function() { 
  $('#btnPrint').on('click', function(e) {
        ...
        var doc_print = $("#doc_print").val();
        var arr=initSearch();
						
       	$.ajax({
                    type: "POST",
                    url: "lib/print_debet.php",
			        data: { doc_print:doc_print, chp_laws:laws },
                    cache: false,
                    success: function(responce){ 
					    var str=responce.replace( /\s/g,'%20');
					    content= "data:application/msword,"+'\uFEFF'+str;
					    var link = document.createElement("a");    
                        link.href = content;
                      //set the visibility hidden so it will not effect on your web-layout
                        link.style = "visibility:hidden";
                        link.download = "111.doc";    
    ...
    var validator = $("#userForm").validate({
        ...
        messages: {
            ...
            pass: {
                ...
                rangelength: jQuery.format("Не менее {0} символов")
            },
            pass_confirm: {
...
$(function () {
    // validate signup form on keyup and submit
    var validator = $("#addRegister").validate({
        rules: {
            rdate: "required",
			provider: "required",
			service:  "required",
			register_file: "required",
			rname: "required"
        },
        messages: {
            rdate: "Введите дату реестра",
			provider: "Выберите поставщика",
			service:  "Выберите услугу",
			register_file: "Выберите файл реестра",
			rname: "Введите название реестра"
...
$(function () {
    ...
    var validator = $("#custForm").validate({
        ...
        messages: {
            family: "Введите фамилию",
            iname: "Введите имя",
			telmobil: "Введите мобильный телефон",
            email: {
                required: "Неправильный email",
                minlength: "Неправильный email"
...
$(function () {
    ...
    var validator = $("#signupform").validate({
        ...
        messages: {
            family: "Enter your firstname",
            iname: "Enter your lastname",
            username: {
